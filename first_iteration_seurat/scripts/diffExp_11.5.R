...
# Read the data
data.wt <- Read10X(data.dir = E11.5.wt.data.dir)
data.ko <- Read10X(data.dir = E11.5.ko.data.dir)

# Create the Seurat objects
E11.5_wt.obj <- CreateSeuratObject(counts = data.wt)
E11.5_ko.obj <- CreateSeuratObject(counts = data.ko)
...
png(file.name)
VlnPlot(E11.5_wt.obj, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
dev.off()

# Three features KO
file.name <- paste0(prefix, "_ko_unfiltered_features.png")
png(file.name)
VlnPlot(E11.5_ko.obj, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
dev.off()
...
png(file.name)
plot1 <- FeatureScatter(E11.5_wt.obj, feature1 = "nCount_RNA", feature2 = "percent.mt")
print(plot1)
dev.off()
...
png(file.name)
plot1 <- FeatureScatter(E11.5_ko.obj, feature1 = "nCount_RNA", feature2 = "percent.mt")
print(plot1)
dev.off()
...
png(file.name)
plot2 <- FeatureScatter(E11.5_wt.obj, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
print(plot2)
dev.off()
...
png(file.name)
plot2 <- FeatureScatter(E11.5_ko.obj, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
print(plot2)
dev.off()
...
E11.5_wt.obj <- subset(E11.5_wt.obj, subset = nFeature_RNA > nFeature_lower_threshold & nFeature_RNA < nFeature_upper_threshold & nCount_RNA > nCount_lower_threshold & nCount_RNA < nCount_upper_threshold & percent.mt < percent_mt_threshold)

# KO dataset filtering
E11.5_ko.obj <- subset(E11.5_ko.obj, subset = nFeature_RNA > nFeature_lower_threshold & nFeature_RNA < nFeature_upper_threshold & nCount_RNA > nCount_lower_threshold & nCount_RNA < nCount_upper_threshold & percent.mt < percent_mt_threshold)
...
# Merge the datasets
merged_obj <- merge(E11.5_wt.obj, y = E11.5_ko.obj, add.cell.ids = c("WT", "KO"), project = "E11.5")
